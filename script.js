       // Sistema de armazenamento (substituindo localStorage para compatibilidade)
        let tasksData = [];
        let currentFilters = {
            status: 'all',
            category: 'all',
        ...
        // Simulação do localStorage para compatibilidade
        const storage = {
            getItem: (key) => {
                if (key === 'tasks') {
                    ...
                    <div class="task-item ${task.completed ? 'completed' : ''} priority-${task.priority}">
                        <div class="task-header">
                            ...
                            <div class="task-actions">
                                <button class="action-btn btn-complete" onclick="app.toggleComplete(${task.id})">
                                    ${task.completed ? '↩️' : '✅'}
                                </button>
                                <button class="action-btn btn-edit" onclick="app.editTask(${task.id})">✏️</button>
                                <button class="action-btn btn-delete" onclick="app.deleteTask(${task.id})">🗑️</button>
                            </div>
                        </div>
                        ...
                        <div class="task-meta">
                            ...
                            ${task.dueDate ? `<span>⏰ Vence: ${this.formatDate(task.dueDate + 'T00:00:00')}</span>` : ''}
                        </div>
                    </div>
                `).join('');
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Carregar algumas tarefas de exemplo se não houver nenhuma
            if (app.tasks.length === 0) {
                const exampleTasks = [
                    ...
                    },
                    {
                        ...
                        description: 'Revisar conceitos de promises e async/await',
                        category: 'estudos',
                        priority: 'high',
                        dueDate: '2025-05-27',
                        completed: false,
                    ...
                    },
                    {
                        ...
                        description: 'Apresentar relatório mensal do projeto',
                        category: 'trabalho',
                        priority: 'medium',
                        dueDate: '2025-05-29',
                        completed: true,
        ...
        // Melhorar a renderização para mostrar tarefas vencidas
        TodoApp.prototype.renderTasks = function(searchQuery = '') {
            const tasksList = document.getElementById('tasksList');
            const filteredTasks = this.getFilteredTasks(searchQuery);
